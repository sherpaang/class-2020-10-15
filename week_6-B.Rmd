...
tibble(urn_red = seq(100, 700, by = 100)) %>% 
  mutate(paddle_red = map(urn_red, ~ rbinom(n = 100,
                                            size = 25, prob = ./1000))) %>% 
# rbinom just flips the coin a bunch of times; it returns either success or
# failure, n = number of tests to do, size = number of samples to run each size
# number of heads and tails, prob = chance of getting a head
...
scene_1 <- tibble(urn_red = 1:1000) %>% 
  mutate(paddle_red = map(urn_red, ~ rbinom(n = 50,
                                            size = 25, prob = ./1000))) %>%
  unnest(paddle_red)
